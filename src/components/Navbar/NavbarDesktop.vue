    ...
    >
      <div class="flex-grow flex items-center w-auto mx-4">
        <div class="flex items-center flex-shrink-0 mr-6">
          <span class="font-semibold font-serifDisplay text-4xl">{{ $static.metadata.shortSiteName }}</span>
        </div>
        <div class="flex-grow">
          <ul class="list-none flex justify-left">
            <li v-for="navItem in $static.metadata.headerNavigation" :key="navItem.name" class="px-4 py-1 font-serif">
              <g-link
                class="block py-1"
                :to="navItem.link"
                :title="navItem.name"
